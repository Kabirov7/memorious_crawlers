...
def get_all_contracts(context, data):
	...
	f = open("all_page.html", "w")
	f.write(response.text)
	f.close()

	data['contracts_num'] = response.html.xpath(const.X_PATH_CONTRACT_NUMBER)
	data['contracts_ids_for_request_form'] = response.html.xpath(const.CONTRACTS_IDS_FOR_REQUEST_FORM)
	data['procuring_organization'] = response.html.xpath(const.X_PATH_PROCURING_ORGANIZATION)
	data['provider'] = response.html.xpath(const.X_PATH_PROVIDER)
	data['contract_type'] = response.html.xpath(const.X_PATH_TYPE)
	data['date'] = response.html.xpath(const.X_PATH_DATE)
	data['contract_number_form'] = contract_number_form
	data['table_val'] = table_val
	data['view_state'] = view_state

	print("CONTRACTS NUMBER IN GET ALL CONTRACTS: ", len(data['contracts_ids_for_request_form']))
	context.emit(rule='get_one_contract', data=data)
	print("EMITED")


def get_one_contract(context, data):
	contracts_number = len(data['contracts_ids_for_request_form'])
	print("CONTRACTS NUMBER: ", contracts_number)
	for i in range(contracts_number):  # for every contract
		try:
			date = datetime.strptime(data['date'][i], "%d.%m.%Y")
		except Exception as e:
			print('Date Exception', e)
			date = data['date'][i]
		try:
			general_info = {
				'contract_number': data['contracts_num'][i],
				'procuring': data['procuring_organization'][i],
				'provider': data['provider'][i],
				'type': data['contract_type'][i],
				'date': date
			}
		except Exception as e:
			print(e)
		...
		new_data = {
			"contracts_ids_for_request_form": data["contracts_ids_for_request_form"][i],
			"contract_number_form": data["contract_number_form"],
			"table_val": data["table_val"],
		...
		print('iteration', new_data['iteration'])
		context.emit(rule='parse_contract', data=new_data)
		print("EMITTED TO PARSE!")


# single_table.append(single_proc)
def parse_contract(context, data):
	print('data["iteration"]', data["iteration"])
	...
	data["contracts_ids_for_request_form"] = \
		data["contracts_ids_for_request_form"].split('{')[-1].split('}')[0].split("'")[1]

	form_preview = {
		data['table_val'].split(':')[0]: data['table_val'].split(':')[0],
		data['contract_number_form']: '',
		'form:supplier_input': '',
		'form:supplier_hinput': '',
		'form:dateFrom_input': '',
		'form:dateTo_input': '',
		data['table_val'] + '_rppDD': '',
		data['table_val'] + '_selection': int(data["general_info"]["contract_number"].split("-")[-1]),
		'javax.faces.ViewState': data['view_state'],
		data["contracts_ids_for_request_form"]: data["contracts_ids_for_request_form"],
	}
	contract_response = _fetch_post(context, data, const.SEED_LINK, form_preview)
	...
	if lot_info:
		...
		f = open("bad_page.html", "w")
		f.write(contract_response.text)
		f.close()
	...
	lot_line = {
		'contract_number': data["general_info"]["contract_number"],
	...
	data["general_info"]["lot_num"] = lot_num
	print("LOTLINE", lot_line)

	lot_info_procurement.append(lot_line)
	...
	del_doc_names = download_file(del_doc_name, del_doc_url, 'delivery', data["general_info"]["contract_number"],
								  del_number)
	...
	graph_doc_names = download_file(graph_doc_name, graph_doc_url, 'payment', data["general_info"]["contract_number"],
									graph_number)

	print("okgz_made_in", okgz_made_in)
	...
	lot_procurement.append(data["general_info"])

	# print("lot_info_procurement")
	# pprint(lot_line)
	# print()
	# pprint("lot_procurement")
	# pprint(lot_procurement)
	contract_data = {}
	contract_data["lot_procurement"] = lot_procurement
	contract_data["lot_info_procurement"] = lot_info_procurement
	contract_data["okgz_classes_procurement"] = okgz_classes_procurement
	contract_data["delivery_time_procurement"] = delivery_time_procurement
	contract_data["payment_schedule_procurement"] = payment_schedule_procurement
	contract_data["payment_files_procurement"] = payment_files_procurement
	contract_data["delivery_files_procurement"] = delivery_files_procurement

	# contract_data = {
	# 	"lot_procurement": lot_procurement,
	# 	"lot_info_procurement": lot_info_procurement,
	...
	_fetch_post(context, data, const.SEED_LINK, form_preview)  # fake request


def save_data(context, data):
	for lot in data['lot_procurement']:
		context.emit(data={"lot_procurement": lot})

	for lot in data['lot_info_procurement']:
		context.emit(data={"lot_info_procurement": lot})

	for lot in data['okgz_classes_procurement']:
		context.emit(data={"okgz_classes_procurement": lot})

	for lot in data['delivery_time_procurement']:
		context.emit(data={"delivery_time_procurement": lot})

	for lot in data['payment_schedule_procurement']:
		context.emit(data={"payment_schedule_procurement": lot})

	for lot in data['payment_files_procurement']:
		context.emit(data={"payment_files_procurement": lot})

	for lot in data['delivery_files_procurement']:
		context.emit(data={"delivery_files_procurement": lot})
