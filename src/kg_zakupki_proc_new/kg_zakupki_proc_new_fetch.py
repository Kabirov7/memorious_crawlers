...
def get_fifty_contracts(context, data):
	step = 50
	for i in range(0, data["count_contracts"], step):
...
def get_one_contract(context, data):
	...
	for i in range(contracts_number):  # for every contract
		...
		context.emit(rule='parse', data=new_data)
	# print("EMIT")
...
def parse_contract(context, data):
	...
	if lot_info:
		...
		f = open(f'{data["general_info"]["contract_number"]}.html', 'w')
	...
	# context.emit(rule="pass", data={
	# 	"lot_procurement": data["general_info"],
